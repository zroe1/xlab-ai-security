...
estabilished attack methods such as the those you implemented in the previous
section.
...
requirement of $C(x + \delta) = t$ differentible. By default, $C(x + \delta)$ returns
an integer which represents a class. This function is not even continuous and certainly not differentiable.
...
Above, we mention that we can add $f$ as a component of the loss we want to minimize.
...
paper has the $L_2$ ditance metric squared instead of the vannilla $L_p$ norm shown above. In the
