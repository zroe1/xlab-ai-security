...
## How Does the Carlini-Wagner Work?
...
attacks. **For this page and the coding excercises, we focus on the $L_2$ attack.** The attack design
...
This formalization is slightly different from your implementions in the previous
section but the main idea should be familiar.

### Change #1: Making $C(x + \delta)$ Differentiable

The first change that Carlini and Wagner make to this objective is making the
requirement of $C(x + \delta) = t$ differentible. By default, $C(x + \delta)$, which
gives a classification is not even connected and certainly not differentiable.
...
We can now change our original equation to the one below where $f$ can be any function where $f(x + \delta) \leq 0$ implies that the model predicts
...
### Change #2: Adding $f(x + \delta)$ And The Constant $c$ to The Loss

Recall that $f$ is only positive if the adversarial example leads to misclassification.
That means that we can use $f$ as a loss term to minimize. Using $f$ as a loss term,
we can change our previous equation to the below where $c$ weights how much $f$
...
$c$ values, the $L_2$ distance metric, and equation $f_6$ from the original paper.
...
The next issue to deal with is the box contraint: how do we keep the images between
0 and 1. The authors deal with this by introducing a change in variables. This step
is a bit confusing so let's start with some mathematical intution. Let a given
pixel component in the adversarial image be $x_i + \delta_i$ where $x_i$ is the original
value and $\delta_i$ is the adversarial perturbation we will add to that pixel compoent.
Now, let $x_i + \delta_i$ be a funcion of $w_i$ which can be any positive
...
and guarentee that we will be left with a valid image.
...
3. Guarentee that $x + \delta$ is between 0 and 1 without any clipping.

<b>Disclaimer: </b>This attack is correct for the $L_2$ situation but the norm looks a bit different
in the $L_\infty$ case and the $L_0$ attack is quite different.{" "}

## Final comments

If any of the math above is confusing to you, there is nothing to worry about.
When you complete the coding excercises everything should become more concrete.
After you are finished with the coding excercises we recommend you read back through
this document to test you knowledge and make sure that you understand everything.
