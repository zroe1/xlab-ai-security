...
context about the model you will be loading. We reccomend you do not skip this section,
...
per image). Each value $x_{ijz}$ in an image $X$ is contrained such that $0 \leq x_{ijz} \leq 1$.
...
which was produced by the US government. Interestingly, depite being incredibly popular, some
important details related to it's construction were never documented and remain unknown [@NEURIPS2019_51c68dc0].
...
contrained such that $0 \leq x_{ij} \leq 1$.
...
In the defensive distilaiton section of this course, you will be using an MLP MNIST
model which you will train from scratch. The reason why you will be using the MNIST
dataset on this section is because it is much easier to train a small model for
MNIST classification rather than CIFAR classification. As a sidenote, researchers
today use MNIST sparingly for adversarial robustness research because models trained
on the dataset are in general too easy to break and results don't generize to other
settings.
