...
While this may seem like an obvious problem, many papers have been published in credible conferences that show high robustness, but under a more comprehensive benchmark, their performance slips. To address this issue Francesco Croce and Matthias Hein (University of Tubingen) proposed AutoAttack (Hein and Croc, 2020). Croce and Hein applied Auto Attack to published defenses and found that the robust accuracy dropped by more than 10% in 13 cases. This illustrates both the difficultly in evaluating ones own defenses and in comparing the effectiveness of defenses across papers.

The next year, Croce and Hein (along with other authors[^1]) followed up AutoAttack with RobustBench, to make evaluation and comparison of defenses easier for the research community. Robustbench uses AutoAttack to evaluate defenses and uses a public leaderboard to track the research community’s progress.
...
3. Models submitted must “not have an optimization loop in the forward pass.”
   ...
   Even if there are non-zero gradients through the forward, the backward pass will be very expensive to calculate.
