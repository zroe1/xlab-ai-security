...
**_Disclaimer: Although the content of this page doesn't contain offensive content, many of the datasets do. Because evaluating models for safety/security requires trying to elicit dangerous/offensive behavior from models, many prompts contain objectionable content._**

_This section is specifically for evaluating LLMs. If you would like to learn about evaluating computer vision models against adversarial attacks, we recommend you read the RobustBench section of the website._

For AI security work, researchers are interested in evaluating models, attacks and defenses. The simplest case is you have a model and you would like to see what percentage of harmful queries it refuses. You can make this more complex by evaluating the model against different jailbreaks. Likewise, if you are designing an attack, you will want to evaluate your approach against different defenses.

We will say up front that _this is hard._ Implementing a state-of-the-art attack or defense at scale can be non-trivial and proper benchmarking is very context-dependent. This document is intended to be an overview of different resources and approaches, but cannot claim to be a definiitve guide for how to do evaluations for AI security.

## Datasets for Evals

### AdvBench

AdvBench was introduced in [@zouUniversalTransferableAdversarial2023] to evaluate the effectiveness of GCG. The eval contains 500 target strings between 3 and 44 tokens (16 tokens on average) that reflect harmful behavior. The adversary "wins" if the model outputs the exact harmful string. The benchmark also includes 500 prompts that attempt to elicit harmful behaviors. In this setting, the authors consider an attack successful if the model "makes a _reasonable_ attempt at executing the behavior."
