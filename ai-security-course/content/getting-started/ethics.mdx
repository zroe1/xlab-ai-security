...
Some AI security research is done on image classifiers which in general are safe to release. However, if you are fine-tuning a language model to remove safety filters or increase its ability to do harmful tasks, this can quickly become ethically problematic. These models may be able to cause harm in the world or could be be misused to generate harmful data. For state-of-the-art language models, _we do not recommend you do this kind of fine-tuning_.
