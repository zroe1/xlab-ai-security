...
@sharmaConstitutionalClassifiersDefending2025 trained LLM-based classifiers (in their case, the line of Claude models) on constitution-guided synthetic data to protect and block jailbreaks. The constitution specifies permitted and restricted categories in natural language, providing flexibility (the authors note that the permitted category is important to limit overrefusal; their first iteration struggled with a very high false-positive rate). To generate the synthetic data, they first use a helpful-only model to create both harmful and harmless inputs and outputs. The authors then apply a diversification system (including translation, paraphrasing, and jailbreaking methods) and automated red-teaming.
...
Constitutional classifiers represent one of the state-of-the-art approaches for improving the safety of LLMs, and do so in a flexible way. Interestingly, constitutional classifiers can be used in a safeguard pipeline, motivated by the [swiss cheese model](https://en.wikipedia.org/wiki/Swiss_cheese_model) of security. But what might a SOTA technique applied directly to the model look like? In the next section, perhaps we'll find out.
