...
The authors then fine-tune Llama-2 on Anthropic's [helpfulness and harmfulness dataset](https://huggingface.co/datasets/Anthropic/hh-rlhf) with their own labeling, following Llama Guard's task structure (note: we believe the citation in the original Llama Guard paper is incorrect).
