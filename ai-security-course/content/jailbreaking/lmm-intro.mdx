...
The transformer architecture was introduced by @vaswani2023attentionneed for language translation. Since then, this architecture has been adapted for more general language modeling and underpins almost all LLMs developed today.
...
A useful loss should maximize the probability of the correct token. The first step we take to accomplish this is computing the log of the probability of the correct token. The graph below shows $\log x $ for $x \in (0, 1)$.
