...
We then return a response consistent with the vote. If the vote is that a jailbreak occurred, we return a sampled response that _agrees that $P$ is a jailbreak prompt_ and thus rejects the query. If the vote is that a jailbreak didn't occur, we return a sampled response that _agrees $P$ is not a jailbreak prompt_, answering the query. The pseudocode for SmoothLLM is given below.
