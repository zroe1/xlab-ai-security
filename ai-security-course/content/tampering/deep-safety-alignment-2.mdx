...
Alright, maybe it's not exactly *simple,* but it's definitely not as complicated as it looks at first glance. For starers, let's look at how the authors define the vanilla supervised fine-tuning (SFT) objective using the same notation:
$$
...
$$
\nabla S(\beta_t \Delta_t) = \frac{e^{\beta_t \Delta_t}}{1 + e^{\beta_t \Delta_t}} \cdot \cdot - \beta_t \nabla \log \pi_\theta(y_t | \mathbf{x}, \mathbf{y}_{< t}).
$$
