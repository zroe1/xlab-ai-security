---
title: "A Few Warnings About Evaluating Open-Weight LLM Safeguard Durability"
...
<p align="center">
  <ThemeImage
    ...
    style={{ align: "center", width: "65%", display: "block", margin: "0 auto" }}
  />
</p>
...
## The Past Mistakes
Back at the height of the adversarial examples crazy in the latter-half of the 2010s, there was a very common technique to supposedly defeat adversarial attacks against computer vision (CV) models: obfuscated gradients. You might recall that we touched on this concept in the RobustBench notebook, but we'll include a brief review here since it's been quite a while. In short, this method involves attempting to remove the usefulness of the gradient in the CV model to greatly increase the difficulty of optimizing the adversarial example. (The wiggly ReLU defense you investigated in the RobustBench notebook was exemplary of this.) As demonstrated by @athalye2018obfuscatedgradientsfalsesense, however, these defenses don't actually get to the root of the problem (they completely broke 6 of 7 obfuscated gradient defenses and partially broke the remaining one). These "defenses" don't work because they don't solve the fact that the CV models are learning brittle representations of the images classes, but rather just make the adversarial optimization process hard. 

Unfortunately, it seems that many of the tamper-resistant attacks proposed in the open-weight LLM literature are falling into similar traps. They don't "unlearn" the model's knowledge of harmful content at all, but just hide it under noisy gradients. In an interview on the Cognitive Revolution Podcast, Nicholas Carlini (who actually third authored the paper we covered in this writeup) expressed his thoughts on these style of defenses:

> This [the obfuscated gradients trend] continued even all the way through to these unfine-tunable models, where one of the papers was explicitly saying "we make the gradients very challenging so that when you train the model, the gradients are ugly and you can't fine-tune the model." This is literally the exact same argument that people were presenting in 2017 for image adversarial examples and it fails in the exact same way. You change the learning rate a little bit, you add some random restarts, and you add some warmup so that things work better. The gradient ends up becoming smooth enough that you can now do optimizations, and then deep learning takes over and the rest is easy. ([Source](https://www.youtube.com/watch?v=fyGdlnUDOdg))

### The Real Lesson
The field is making the exact same mistakes as before! This paper hopefully makes it very clear that defenses should be *thoroughly tested* and framed accurately if they're going to be proposed as a viable defense. Unlearning methods *should not* just obfuscate gradients, but should *thoroughly and completely* remove the unwanted knowledge from the model (think distillation robustifies unlearning). Ultimately, however, if we want to build safe and tamper-resistant open-weight LLMs, there's no better approach than making sure these models are safely pre-trained from the start [@mainiSafetyPretrainingNext2025; @obrienDeepIgnoranceFiltering2025].
