...
## Why is safety training so brittle?
...
<p align="center">
  <ThemeImage
    lightSrc="/images/neuron_extraction.png"
    darkSrc="/images/neuron_extraction.png"
    alt="Safety Neuron Extraction Technique"
    style={{ align: "center", width: "80%", display: "block", margin: "0 auto" }}
  />
</p>

<div align="center">
  **Fig. 1** <br></br> Methodology for extracting safety-critical neurons; Figure 1 from @weiAssessingBrittlenessSafety2024
</div>
...
$$
This is just the first-order Taylor approximation of the change in loss when the entry $W_{ij}$ is set to 0, but let's look into this a bit more closely.

Backing up, given a differentiable function $\mathcal{L}(w)$, we can estimate the change in $\mathcal{L}$ given a small change in input $\delta w$ with a first-order Taylor expansion, which is a fancy way of saying we're gonna multiply the change in $w$ by the slope to get the change in $\mathcal{L}(w)$. Specifically,
$$
...
<p align="center">
  <ThemeImage
    lightSrc="/images/rank_extraction.png"
    darkSrc="/images/rank_extraction.png"
    alt="Safety Rank Extraction Technique"
    style={{ align: "center", width: "80%", display: "block", margin: "0 auto" }}
  />
</p>

<div align="center">
  **Fig. 2** <br></br> Methodology for extracting safety-critical ranks; Figure 1 from @weiAssessingBrittlenessSafety2024
</div>
...
## Conclusion
Fine-tuning attacks are an extremely simple but highly potent method of removing safety training in models. Their efficacy seems to partly stem from the sparseness of safety-critical ranks and neurons in models, but we'll hold off on further analysis for now. First, we'll touch on a specific kind of fine-tuning attack dubbed "Emergent Misalignment" and then look into some white-box model defenses.
