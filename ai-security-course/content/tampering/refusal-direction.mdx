...
## Background
From previous word, we know that word (or token) embeddings encode semantically meaningful differences between themselves [@mikolovLinguisticRegularitiesContinuous2013]. For example, taking the vector for `King`, subtracting `Man`, and then adding `Woman` gets a vector close to `Queen` (if this concept is unfamiliar to you, 3blue1brown has made a [great visualization](https://youtube.com/shorts/FJtFZwbvkI4?si=dF8DIFeSScgEHc-c) of it). This has been a well-known result for over a decade, but it has not often been applied to more abstract concepts, such as "refusal." This is exactly what @arditiRefusalLanguageModels2024 look into in their landmark interpretability paper "Refusal in Language Models is Mediated by a Single Direction."

## Finding the Refusal Direction
In order to find the direction associated with refusal, as in @arditiRefusalLanguageModels2024 we first construct a dataset of harmless prompts (which are not refused by the model) and a dataset of harmful prompts (which are refused by the model). We then then run all the prompts in each dataset through our target model, calculating the mean activation (hidden state) for each layer in the model and each post-instruction token. (Post-instruction tokens are the template tokens between the user query and model response; @arditiRefusalLanguageModels2024 use them for their standardization and hypothesizing that they contain information about how the model will formulate its response.) 
...
We can add our refusal vector to the activations of an input to induce refusal on it:
$$
