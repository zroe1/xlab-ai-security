...
class WhiteBox1(nn.Module):
    """WideResNet architecture - Ultra-compact WideResNet for MNIST with minimal parameters"""
    def __init__(self, num_classes=10, width_multiplier=2, dropout_rate=0.3):
        super(WhiteBox1, self).__init__()
    ...
    def _make_group(self, in_channels, out_channels, num_blocks, stride, dropout_rate):
        layers = []
        layers.append(self._BasicBlock(in_channels, out_channels, stride, dropout_rate))
        for _ in range(1, num_blocks):
            layers.append(self._BasicBlock(out_channels, out_channels, 1, dropout_rate))
        return nn.Sequential(*layers)
...
class WhiteBox2(nn.Module):
    """Simple CNN for MNIST classification"""
    def __init__(self, num_classes=10, dropout_rate=0.3):
        super(WhiteBox2, self).__init__()
